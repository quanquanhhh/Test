    ...
    internal sealed class Fsm<T> : FsmBase, IFsm<T> where T : class
    {
        ...
        public static Fsm<T> Create(string name, T owner, params FsmState<T>[] states)
        {
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                {
                    throw new Exception(FUtility.Text.Format("FSM '{0}' state '{1}' is already exist.", new TypeNamePair(typeof(T), name), stateType.FullName));
                }
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            ...
            if (state == null)
            {
                throw new Exception(FUtility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), typeof(TState).FullName));
            }
        ...
        public void Start(Type stateType)
        {
            ...
            {
                throw new Exception(FUtility.Text.Format("State type '{0}' is invalid.", stateType.FullName));
            }

            FsmState<T> state = GetState(stateType);
            if (state == null)
            {
                throw new Exception(FUtility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), stateType.FullName));
            }
        ...
        internal async void ChangeState(Type stateType)
        {
            if (m_CurrentState == null)
            {
            ...
            if (state == null)
            {
                throw new Exception(FUtility.Text.Format("FSM '{0}' can not change state to '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), stateType.FullName));
            }

            await m_CurrentState.PreOnLeave(this, state);
            m_CurrentState.OnLeave(this, false);
            m_CurrentStateTime = 0f;
            m_CurrentState = state;
            await m_CurrentState.PreOnEnter(this);
            m_CurrentState.OnEnter(this);
        }
    }
}